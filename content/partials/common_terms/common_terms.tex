\section{Common Terms}
    ...
    \begin{enumerate}
        ...
        \bitem{PyPy\footnote{PyPy is a Python interpreter written in Python}:}\\
    ...
    \subsection{Common Modules From The Standard Library}
        ...
        \begin{enumerate}
            \bitem{os:} interaction with the operating system like accessing the file system
            \bitem{sys:} information about the file system
            \bitem{re:} working with regular expressions
            \bitem{tkinter:} creation of graphical user interfaces
            \bitem{itertools:} provides efficient iterators
            \bitem{functools:} provides higher-order functions
            \bitem{pickle:} for serialization
            \bitem{argparse:} makes it easy to parse and specify command line arguments
        \end{enumerate}
    ...
    \subsection{Common Third-Party Modules}
        ...
        \begin{enumerate}
            \bitem{SciPy:} used for scientific and technical computing; contains different modules, one of the most popular ones is NumPy. 
            \bitem{NumPy:} provides fast (multi-dimensional) arrays with many handy features and functions. One NumPy array has exactly one data type.
            \bitem{pandas:} mainly used because of their dataframes; you think of dataframes as (e.g. Excel) spreadsheets, so two-dimensional arrays where every column can have a different data type.
            \bitem{matplotlib:} used for plotting graphs (e.g. line plots, scatter plots, histograms, heat maps)
        \end{enumerate}
